...
function linkFilter() {
	...
	const regUrlOne = /^\s*https?:\/\/\w+\.\S+$/;
	const regUrlTwo = /^\s*www\.\w+\.\S+$/;
	const regUrlThree = /^\s*\w+\.\w+\S+$/;
	const regUrlPrefix = /^https?:\/\//;

	for (let i = 0; i < arrayOfLinks.length; i++) {
		arrayOfLinks[i] = arrayOfLinks[i].trim();
		if (regUrlOne.test(arrayOfLinks[i])) {
			urlsContainer.push(arrayOfLinks[i]);
		} else if (regNumber.test(arrayOfLinks[i])) {
			let ip = arrayOfLinks[i].split(/\./);
			if (parseInt(ip[0]) < 256 & parseInt(ip[1]) < 256 & parseInt(ip[2]) < 256 & parseInt(ip[3]) < 256) {
				urlsContainer.push('http://' + arrayOfLinks[i]);
			}
		} else if (regUrlTwo.test(arrayOfLinks[i])){
			urlsContainer.push('http://' + arrayOfLinks[i]);
		} else if (regUrlThree.test(arrayOfLinks[i])){
			urlsContainer.push('http://' + arrayOfLinks[i]);
		}
	}
	urlsContainer.sort( function(a, b){
		if (b.replace(regUrlPrefix, '') < a.replace(regUrlPrefix, '')) { return 1;}
		if (b.replace(regUrlPrefix, '') > a.replace(regUrlPrefix, '')) { return -1;}
		return 0;
	});
	for (let i = 0; i < urlsContainer.length; i++) {
		...
		linkRef.innerHTML = urlsContainer[i].replace(regUrlPrefix, '') + '<br>';
		linkRef.href = urlsContainer[i];
